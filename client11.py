...
from instruction_screen import Ui_InstructionWindow, show_instructions_win
...
instructions_window = Ui_InstructionWindow(client11)
round_num = 0
...
while socket_running:
    ...
    while client11.client_ready is False:
        if client11.screen_state == 4:
            ...
            ready_window.instructions_button.show()
        if client11.screen_state == 5:
            ...
            ready_window.tutorial_button.show()
            ready_window.instructions_button.show()
        if client11.screen_state == 6:
            try:
                t39 = th.Thread(target=show_instructions_win, args=(instructions_window,))
                t39.start()
            except:
                pass
            while client11.screen_state == 6:
                pass
            ready_window.voice_button.show()
            ready_window.leaderboard_button.show()
            ready_window.tutorial_button.show()
    ...
    while round_num < 3:
        ...
        while cmd != "10":
            ...
            if client11.voice is True:
                voice_to_text()
                try:
                    game_window.print_word_to_screen(MyText)
                except:
                    pass
        ...
        for i in range(4):
            game_window.insert_value(i, round_num, "xxx")
        game_window.game_table.update()
        if cmd == "45":
